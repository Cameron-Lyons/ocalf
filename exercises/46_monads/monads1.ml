(* TODO: Implement a Writer monad for logging *)
...
module Writer : sig
  ...
  let return x = ???  (* Wrap x with an empty log *)

  let bind (x, log1) f =
    ???  (* Apply f to x, then concatenate logs *)

  let tell msg = ???  (* Return () with a single-entry log *)

  let run t = ???  (* Return the value and log *)
end
