(* TODO: Define and use extensible variant types *)

type event = ..

type event += ???  (* Add a Click constructor with no payload *)
type event += ???  (* Add a KeyPress of char *)
type event += ???  (* Add a Resize of int * int *)
